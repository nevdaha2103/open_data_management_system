...
1. [Характеристика ділових процесів](#business-processes)
   ...
   - [Запит на доступ до закритих даних](#RequestAccessToClosedData)
...
| ID |	<span id="RequestAccessToClosedData">RequestAccessToClosedData</span> |
| :------------- | :------------- |
| НАЗВА |	Запит на доступ до закритих даних |
| УЧАСНИКИ |	Користувач, Адміністратор |
| ПЕРЕДУМОВИ |	Користувач не має доступу до закритих даних. |
| РЕЗУЛЬТАТ	| Користувач надіслав запит на доступ до закритих даних. |
| ВИКЛЮЧНІ СИТУАЦІЇ |	Відсутні права для запиту.<br/> Помилка підключення до бази даних. |
| ОСНОВНИЙ СЦЕНАРІЙ |	1. Користувач натискає на закриті дані.<br/>2. Система пропонує надіслати запит адміністратору.<br/>3. Користувач заповнює форму запиту.<br/>4. Система надсилає запит адміністратору. |
